// app/lib/catalogMenu.ts

export type CategoryNode = {
  code: string;          // Уникальный код: CLIMATE, WATER, TANKS и т.д.
  name: string;          // Человеческое название (кириллица)
  slug: string;         // ЧПУ-урл (латиница, транслитерация)
...
export function transliterateToLatin(text: string): string {
  ...
  return text
    ...
    .join('')
    .replace(/[^a-z0-9-]+/g, '-') // оставляем только буквы, цифры и дефисы
...
// Основное дерево категорий с автоматической генерацией slug
export const CATEGORY_TREE: CategoryNode[] = (() => {
  // Создаем функцию для рекурсивного добавления slug
  function addSlugs(node: CategoryNode): CategoryNode {
    return {
      ...node,
      slug: node.slug || generateSlug(node.name),
      children: node.children?.map(child => addSlugs(child))
    };
  }
  ...
  return tree.map(node => addSlugs(node));
})();

// Функция для поиска категории по slugs (теперь сравниваем нормализованные slugs)
export function findCategoryBySlugs(slugs: string[]): CategoryNode[] | null {
  if (!slugs.length) return null;

  // Декодируем URL-encoded slugs
  const decodedSlugs = slugs.map(slug => decodeURIComponent(slug));

  function dfs(nodes: CategoryNode[], index: number): CategoryNode[] | null {
    if (index >= decodedSlugs.length) return [];

    const slug = decodedSlugs[index];
    const normalizedSlug = transliterateToLatin(slug); // Нормализуем входящий slug
    ...
    if (index === decodedSlugs.length - 1) {
      return [node];
    }

    const childChain = dfs(node.children ?? [], index + 1);
    if (!childChain) return null;

    return [node, ...childChain];
  }

  return dfs(CATEGORY_TREE, 0);
}

// Функция для построения href из цепочки категорий
export function buildCategoryHrefFromChain(chain: CategoryNode[]): string {
  const parts = chain.map((node) => node.slug);
